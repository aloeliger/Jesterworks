...
class SkimThread(threading.Thread):
    def __init__(self,ThreadNumber,RunFunction,IDQ,OutTreeName,OutFileName,
                 InputChainName,WorkQ,TheCuts,RenameDictionary):
    ...
    def run(self):
        self.RunFunction(self.ThreadNumber,self.IDQ,self.OutTreeName,
                         self.OutFileName,self.InputChainName,
                         self.WorkQ,self.TheCuts,
...
class Jesterworks():
            ...
            for Element in self.Configuration[Token]:                                
                ...
                if(Token == "META"):
                    if(Element == "numthreads"):
                        self.NumThreads = int(self.Configuration[Token][Element])
    ...
    def PerformSkim(self,ThreadNumber,IDQ,OutTreeName,OutFileName,InputChainName,WorkQ,TheCuts,RenameDictionary):
        ...
        while SomethingToDo:
            ...
            if WorkQ.empty():
                ...
                ProcessNumber = IDQ.get()
                FileToSkimName = WorkQ.get()                
                queueLock.release()
                #print("Thread #"+str(ThreadNumber)+" Handling Process#"+str(ProcessNumber))
        #Create the file,
                OutFile = ROOT.TFile(OutFileName+"_"+str(ProcessNumber)+".root", "RECREATE")
        #just create a scratch file for root to have disc access
                
                ScratchFile = ROOT.TFile("Scratch_"+OutFileName.split("/").pop()+"_"+str(ThreadNumber)+".root","RECREATE")
                ScratchFile.cd()
